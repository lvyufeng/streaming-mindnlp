...
from mindnlp.transformers.models.llama.modeling_llama import (
    LlamaAttention,
    rotate_half,
...
def llama_pos_shift_attention_forward(
    ...
    if self.config.pretraining_tp > 1:
        ...
        query_states = [
            ops.dense(hidden_states, query_slices[i])
            for i in range(self.config.pretraining_tp)
        ]
        query_states = ops.cat(query_states, axis=-1)

        key_states = [
            ops.dense(hidden_states, key_slices[i])
            for i in range(self.config.pretraining_tp)
        ]
        key_states = ops.cat(key_states, axis=-1)

        value_states = [
            ops.dense(hidden_states, value_slices[i])
            for i in range(self.config.pretraining_tp)
        ]
    ...
    key_position_ids = ops.arange(kv_seq_len).unsqueeze(0)
    ...
    attn_weights = ops.softmax(attn_weights, axis=-1, dtype=mindspore.float32).to(
        query_states.dtype
    )
    ...
    if self.config.pretraining_tp > 1:
        ...
        attn_output = sum(
            [
                ops.dense(attn_output[i], o_proj_slices[i])
                for i in range(self.config.pretraining_tp)
            ]
        )
    else:
    ...
    for name, module in reversed(model._cells.items()):
        ...
        if isinstance(module, LlamaAttention):
            model._cells[name].construct = types.MethodType(
                llama_pos_shift_attention_forward, model._cells[name]
            )
